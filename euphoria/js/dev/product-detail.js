import { g as bodyUnlock, m as bodyLockStatus, p as bodyLock, s as addToCart } from "./main.min.js";
import "./footer.min.js";
import "./watcher.min.js";
import "./button2.min.js";
import "./tabs.min.js";
import { c as elementChildren, f as isObject, i as Navigation, n as A11y, o as Keyboard, s as Swiper, t as EffectFade } from "./effect-fade.min.js";
...
var Popup = class {
	constructor(options) {
		let config = {
			...
			youtubeTitleAttribute: "data-fls-popup-youtube-title",
	...
	eventsPopup() {
		document.addEventListener("click", function(e) {
			...
			if (buttonOpen) {
				...
				this.youTubeTitle = buttonOpen.getAttribute(this.options.youtubeTitleAttribute) ? buttonOpen.getAttribute(this.options.youtubeTitleAttribute) : null;
		...
		if (bodyLockStatus) {
			...
			if (this.targetOpen.element) {
				...
				if (codeVideo) {
					...
					const titleVideo = this.youTubeTitle || this.targetOpen.element.getAttribute(`${this.options.youtubeTitleAttribute}`) || "YouTube video player";
	...
	_openToHash() {
		...
		if (openButton) {
			...
			this.youTubeTitle = openButton.getAttribute(this.options.youtubeTitleAttribute) ? openButton.getAttribute(this.options.youtubeTitleAttribute) : null;
		}
...
//#endregion
//#region src/components/pages/product-detail/product-detail.js
function initTopGallery() {
	const thumbsEl = document.querySelector("[data-fls-gallery-thumbs]");
	const mainEl = document.querySelector("[data-fls-gallery-main]");
	...
	const thumbsSwiper = new Swiper(thumbsEl, {
		...
		navigation: {
			prevEl: ".top__thumbs-arrow--prev",
			nextEl: ".top__thumbs-arrow--next"
		},
		keyboard: {
			enabled: true,
...
window.addEventListener("load", initTopGallery);
function initAddToCart() {
	const button = document.querySelector(".top__add-to-cart");
	const titleEl = document.querySelector(".top__title");
	const priceEl = document.querySelector(".top__price");
	const imageEl = document.querySelector(".main-gallery__image");
	const statusEl = document.querySelector("[data-cart-status]");
	if (!button || !titleEl || !priceEl) return;
	button.addEventListener("click", () => {
		const title = titleEl.textContent.trim();
		const sizeInput = document.querySelector("input[name=\"size\"]:checked");
		const colorLabel = document.querySelector("input[name=\"color\"]:checked")?.closest(".top__color-item")?.querySelector(".visually-hidden")?.textContent.trim();
		addToCart({
		...
		if (statusEl) statusEl.textContent = `${title} added to cart.`;
	});
}
