import { S as slideUp, _ as dataMediaQueries, b as slideDown } from "./main.min.js";
...
function showMore() {
	...
	function syncItemsInert(showMoreBlock, showMoreContent) {
		if ((showMoreBlock.dataset.flsShowmore ? showMoreBlock.dataset.flsShowmore : "size") !== "items") return;
		const showMoreTypeValue = showMoreContent.dataset.flsShowmoreContent ? showMoreContent.dataset.flsShowmoreContent : 3;
		const isActive = showMoreBlock.classList.contains("--showmore-active");
		const showMoreItems = showMoreContent.children;
		for (let index = 0; index < showMoreItems.length; index++) !isActive && index >= showMoreTypeValue ? showMoreItems[index].setAttribute("inert", "") : showMoreItems[index].removeAttribute("inert");
	}
...
//#endregion
//#region src/components/forms/range/range.js
function rangeInit() {
	document.querySelectorAll("[data-fls-range]").forEach((rangeSlider) => {
		const min = parseFloat(rangeSlider.dataset.flsRangeMin ?? 0);
		const max = parseFloat(rangeSlider.dataset.flsRangeMax ?? 100);
		const prefix = rangeSlider.dataset.flsRangePrefix ?? "";
		const start = (rangeSlider.dataset.flsRangeStart ?? `${min},${max}`).split(",").map((value) => parseFloat(value));
		initialize(rangeSlider, {
			start,
			connect: start.length > 1 ? true : [true, false],
			range: {
				min,
				max
			},
			format: {
				to: (value) => `${prefix}${Math.round(value)}`,
				from: (value) => parseFloat(String(value).replace(prefix, ""))
			}
		});
		const handles = rangeSlider.querySelectorAll(".noUi-handle");
		const labelFrom = rangeSlider.dataset.flsRangeLabelFrom ?? "Minimum value";
		const labelTo = rangeSlider.dataset.flsRangeLabelTo ?? "Maximum value";
		handles.forEach((handle, index) => {
			handle.setAttribute("aria-label", handles.length > 1 && index === 1 ? labelTo : labelFrom);
		});
		const inputs = ["from", "to"].map((key) => {
			const selector = rangeSlider.dataset[`flsRange${key[0].toUpperCase()}${key.slice(1)}`];
			return selector ? document.querySelector(selector) : null;
		});
		if (inputs.some(Boolean)) {
			rangeSlider.noUiSlider.on("update", (values) => {
				inputs.forEach((input, index) => {
					if (input && values[index] !== void 0) input.value = values[index];
				});
			});
			inputs.forEach((input, index) => {
				if (!input) return;
...
document.querySelector("[data-fls-range]") && window.addEventListener("load", rangeInit);
//#endregion
