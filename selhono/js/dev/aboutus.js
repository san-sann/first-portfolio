...
function syncPagination(swiper, dots) {
...
function initSliders() {
	const sliderEl = document.querySelector("[data-fls-slider]");
	if (sliderEl) {
		const prefersReducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
		const dots = Array.from(sliderEl.querySelectorAll(".pageintro__dot"));
		const swiper = new Swiper("[data-fls-slider]", {
			modules: [
				Navigation,
				Keyboard,
				A11y
			...
			on: {
				init: (swiper) => syncPagination(swiper, dots),
				slideChange: (swiper) => syncPagination(swiper, dots)
			}
		});
		dots.forEach((dot, index) => {
