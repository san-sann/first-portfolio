import { o as slideToggle, s as slideUp } from "./main.min.js";
...
var SelectConstructor = class {
	...
	selectInit(originalSelect, index) {
		...
		if (originalSelect.options.length) {
			...
			if (this.getSelectPlaceholder(originalSelect) && this.getSelectPlaceholder(originalSelect).label.show) {
				const selectItemTitle = this.getSelectElement(selectItem, this.selectClasses.classSelectTitle).selectElement;
				if (selectItemTitle) selectItemTitle.insertAdjacentHTML("afterbegin", `<span class="${this.selectClasses.classSelectLabel}">${this.getSelectPlaceholder(originalSelect).label.text ? this.getSelectPlaceholder(originalSelect).label.text : this.getSelectPlaceholder(originalSelect).value}</span>`);
			}
	...
	selectsActions(e) {
		...
		else if (type === "keydown" && (e.code === "Enter" || e.code === "Space") && isTag) {
			e.preventDefault();
			const optionItem = document.querySelector(`.${this.selectClasses.classSelect}[data-fls-select-id="${isTag.dataset.flsSelectId}"] .select__option[data-fls-select-value="${isTag.dataset.flsSelectValue}"]`);
			this.optionAction(selectItem, originalSelect, optionItem);
		}
	}
	...
	getSelectTitleValue(selectItem, originalSelect) {
		...
		if (originalSelect.multiple && originalSelect.hasAttribute("data-fls-select-tags")) {
			selectTitleValue = this.getSelectedOptionsData(originalSelect).elements.map((option) => `<span role="button" tabindex="0" aria-label="${option.textContent.replace(/"/g, "&quot;")} - remove" data-fls-select-id="${selectItem.dataset.flsSelectId}" data-fls-select-value="${option.value}" class="--select-tag">${this.getSelectElementContent(option)}</span>`).join("");
