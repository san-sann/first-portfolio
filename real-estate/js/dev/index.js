...
var introPanelSlot = document.querySelector(".intro__panel-slot");
var heroPanelHost = document.querySelector(".hero__media");
if (heroPanel && introPanelSlot && heroPanelHost) {
	const tabletRange = window.matchMedia("(48em < width <= 62em)");
	const applyPanelPosition = () => {
		if (tabletRange.matches) introPanelSlot.appendChild(heroPanel);
		else heroPanelHost.appendChild(heroPanel);
	};
	tabletRange.addEventListener("change", applyPanelPosition);
	applyPanelPosition();
}
//#endregion
