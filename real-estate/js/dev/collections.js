import { n as bodyUnlock, t as bodyLock } from "./main.min.js";
/* empty css           */
...
function rangeInit() {
	document.querySelectorAll("[data-fls-range]").forEach((rangeSlider) => {
		...
		const start = (rangeSlider.dataset.flsRangeStart ?? `${min},${max}`).split(",").map((v) => parseFloat(v));
		initialize(rangeSlider, {
			...
			tooltips: true,
			format: {
				to: (v) => `${prefix}${Math.round(v)}`,
				from: (v) => parseFloat(String(v).replace(prefix, ""))
			}
		});
		const fromSelector = rangeSlider.getAttribute("data-fls-range-from");
		const toSelector = rangeSlider.getAttribute("data-fls-range-to");
		...
		if (fromInput || toInput) rangeSlider.noUiSlider.on("update", (values) => {
			if (fromInput) fromInput.value = values[0];
			if (toInput) toInput.value = values[values.length - 1];
		});
	});
}
document.querySelector("[data-fls-range]") && window.addEventListener("load", rangeInit);
//#endregion
//#region src/components/pages/collections/collections.js
