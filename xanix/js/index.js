...
//#endregion
//#region src/components/layout/works-slider/works-slider.js
function initWorksSlider() {
	if (document.querySelector("[data-fls-works-slider]")) {
		const reduceMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
		new Swiper("[data-fls-works-slider]", {
...
document.querySelector("[data-fls-works-slider]") && window.addEventListener("load", initWorksSlider);
//#endregion
//#region src/components/layout/testimonials-slider/testimonials-slider.js
function initTestimonialsSlider() {
	if (document.querySelector("[data-fls-testimonials-slider]")) {
		const reduceMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
		new Swiper("[data-fls-testimonials-slider]", {
			modules: [Keyboard, A11y],
			keyboard: {
				enabled: true,
				onlyInViewport: false,
...
document.querySelector("[data-fls-testimonials-slider]") && window.addEventListener("load", initTestimonialsSlider);
//#endregion
...
function menuInit() {
	...
	function closeMenu() {
		if (bodyLockStatus && html.hasAttribute("data-fls-menu-open")) {
			bodyLockToggle();
	...
	document.addEventListener("click", function(e) {
		if (e.target.closest("[data-fls-menu]")) {
			html.hasAttribute("data-fls-menu-open") ? closeMenu() : openMenu();
			return;
		}
		if (e.target.closest("[data-fls-menu-backdrop]")) closeMenu();
	});
	document.addEventListener("keydown", function(e) {
		if (e.key === "Escape" && html.hasAttribute("data-fls-menu-open")) {
			closeMenu();
	...
	if (menuButton && menuBody) {
		const syncState = () => {
			const isOpen = html.hasAttribute("data-fls-menu-open");
			menuButton.setAttribute("aria-expanded", isOpen ? "true" : "false");
			menuButton.setAttribute("aria-label", isOpen ? "Close menu" : "Open menu");
			if (mobileMq.matches) isOpen ? menuBody.removeAttribute("inert") : menuBody.setAttribute("inert", "");
			else menuBody.removeAttribute("inert");
		};
		syncState();
		new MutationObserver(syncState).observe(html, {
			attributes: true,
...
function formRating() {
	...
	if (ratings) ratings.forEach((rating, ratingIndex) => {
		const ratingValue = +rating.dataset.flsRatingValue;
		formRatingInit(rating, +rating.dataset.flsRatingSize ? +rating.dataset.flsRatingSize : 5, ratingIndex);
	...
	function formRatingInit(rating, ratingSize, ratingIndex) {
		const isInteractive = rating.dataset.flsRating === "set";
		const groupName = `rating-${ratingIndex}`;
		let ratingItems = ``;
		for (let index = 0; index < ratingSize; index++) {
			index === 0 && (ratingItems += `<div class="rating__items">`);
			ratingItems += isInteractive ? `<label class="rating__item">
					<input class="rating__input" type="radio" name="${groupName}" value="${index + 1}" aria-label="Rate ${index + 1} star${index + 1 === 1 ? "" : "s"}">
...
function _flsWatcherInit() {
	document.querySelector("[data-fls-watcher]") && new ScrollWatcher({});
}
document.readyState === "loading" ? document.addEventListener("DOMContentLoaded", _flsWatcherInit) : _flsWatcherInit();
//#endregion
