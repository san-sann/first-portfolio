import { a as getHash, c as slideDown, i as dataMediaQueries, s as setHash, u as slideUp } from "./common.min.js";
...
function initSliders() {
	if (document.querySelector("[data-fls-slider]")) {
		...
		new Swiper("[data-fls-slider]", {
			...
			navigation: {
				prevEl: ".widgets__bytes-arrow--prev",
				nextEl: ".widgets__bytes-arrow--next"
			},
			keyboard: {
				enabled: true,
...
//#endregion
//#region src/components/custom/newscard/newscard.js
async function shareNewscard(button) {
	const card = button.closest(".newscard");
	const link = card ? card.querySelector(".newscard__link") : null;
	if (!link) return;
	const title = card.querySelector(".newscard__title");
	const shareData = {
	...
	try {
		...
		const label = button.querySelector(".newscard__action-label");
		if (!label) return;
...
function toggleNewscardSave(button) {
	const saved = button.getAttribute("aria-pressed") === "true";
	button.setAttribute("aria-pressed", String(!saved));
	const label = button.querySelector(".newscard__action-label");
	if (label) label.textContent = saved ? "Read Later" : "Saved";
}
document.addEventListener("click", function(e) {
	const shareButton = e.target.closest("[data-fls-newscard-share]");
	if (shareButton) {
		shareNewscard(shareButton);
		return;
	}
	const saveButton = e.target.closest("[data-fls-newscard-save]");
	if (saveButton) toggleNewscardSave(saveButton);
});
//#endregion
export { Swiper as i, Navigation as n, Keyboard as r, A11y as t };
